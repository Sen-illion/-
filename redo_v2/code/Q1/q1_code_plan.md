...
轮次：`round2`（相邻季次重茬约束修订；`round1` 仅保留历史证据）
...
硬约束还包括地块季次容量、适种范围、水浇地单季水稻/两季蔬菜模式互斥，以及人工修订后的相邻种植周期重茬口径：

- 单季地块的相邻年份视为相邻种植周期；
- 水浇地单季水稻的相邻年份视为相邻种植周期；
- 智慧大棚按“当年第一季→当年第二季→下一年第一季”的顺序，仅禁止同一作物在相邻季次连续出现；
- 普通大棚、水浇地两季蔬菜的相邻季次合法作物集合本身不重叠，不额外扩大为年度禁种；
- 2023 历史按同一时间顺序连接到 2024 的首个相邻周期。

仍采用整块管理二元启用变量：只要某作物在该地块某季种植了正面积，相邻种植周期的整块地均不得再次启用该作物；本轮不追踪地块内部物理子区域的面积交换。
...
2. 豆类年份在合法季次中选择不违反相邻季次重茬、且基准亩利润最高的豆类；必要时在豆类集合中顺次替换。
