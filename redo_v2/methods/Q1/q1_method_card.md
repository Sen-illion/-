...
|---|---|---|---|---|---|---|
| Q1-M1 | 三年窗口 6,573 变量、3,210 整数变量，14.88秒达到0.81%缺口 | 探针曾把智慧大棚第一季复用为第二季参数；人工现已改为复用普通大棚第一季参数，本项证据待刷新 | 41种作物被使用，前5面积占60.86%，浪费率2.13% | ±5%价格探针在20秒内未收敛，分配重合度证据不稳定 | 完整七年单体模型45秒仍有19%–57%缺口；滚动窗口可行 | CONDITIONAL（参数映射已变化） |
| Q1-B1 | 全七年即时生成且硬约束检查为0错误 | 使用同一数据口径 | 仅8种作物、前5占96.56%，明显集中 | 对销量规则高度敏感 | 运行成本低 | CONDITIONAL |

2026-09-03 参数映射审计表明，相对旧探针映射，新规则覆盖的 18 种蔬菜亩产提高 9.09%–12.50%、亩成本降低 7.69%–10.00%、价格中点降低 16.67%–22.79%。三项作用方向不同，不能仅凭单项变化推断利润或种植结构，必须刷新优化探针。证据：`methods/Q1/probes/smart_greenhouse_mapping_audit.json`。
