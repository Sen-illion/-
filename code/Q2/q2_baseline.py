...
def run_baseline():
    ...
    averaged = mean_scenario(scenarios)
    started = time.perf_counter()
    plan, windows = rolling_optimize(
        land, planting, economics, [averaged],
    ...
    save_plan(tables / "result2_baseline.csv", plan, land, crops)
    evaluation.to_csv(tables / "baseline_scenario_annual_metrics.csv", index=False, encoding="utf-8-sig")
    metrics = {
